...
# # 1. face detector + shape predictor
# detector = dlib.get_frontal_face_detector()
# predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")

# def apply_blur(image: np.ndarray, strength: int) -> np.ndarray:
#     if strength % 2 == 0:  # OpenCV blur kernel size must be odd
#         strength += 1
#     return cv2.GaussianBlur(image, (strength, strength), 0)
...
#         # Calculate angles from center point (-180 to 180 degrees)
#         angles = np.arctan2(-(y_coords - center_y),
#                             x_coords - center_x) * 180 / np.pi
...
# # 2. Gradio Interface
# demo = gr.Interface(
#     fn=blur_face_outline,
#     inputs=[
#         gr.Image(type="numpy", label="Upload Image"),
#         gr.Slider(minimum=60,
#                   maximum=80,
#                   value=66,
#                   step=2,
#                   label="Blur Strength",
#                   info="Higher values create stronger blur effect"),
#         gr.CheckboxGroup(
#             choices=["Left", "Right", "Mouth"],
#             value=["Left"],
#             label="Unblur Parts",
#             info="Select parts of face to unblur (multiple selection possible)"
#         )
#     ],
#     outputs=gr.Image(type="numpy", label="Selective Face Focus"),
#     title="Face Focus Effect",
#     description=
#     "Keeps the selected parts of the face clear while blurring the rest of the image. Choose blur strength and areas to unblur.",
# )

# # demo.queue(10).launch(share=True)
